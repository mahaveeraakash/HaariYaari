...
import { Menu, Search, X, ShoppingCart, Leaf, Sprout, Bug, Shield } from 'lucide-react';
import { useNavigate } from 'react-router-dom';
import axios from 'axios';
import { CartContext } from '../context/CartContext';

function Home() {
  const { addToCart, setIsCartOpen, cartItems } = useContext(CartContext);
    ...
    const fetchProducts = async () => {
      try {
        ...
        if (Array.isArray(data)) {
          ...
          throw new Error("Network configuration error. Expected an array.");
        }
      } catch (err) {
    ...
    <div className="min-h-screen flex flex-col bg-mintwhite font-sans text-deepgreen">
      ...
      <nav className="fixed top-0 w-full bg-white shadow-md z-50 transition-all">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
          ...
          <div className="flex items-center gap-4">
            ...
            <button 
              className="p-2 text-deepgreen hover:text-vibrantgreen transition-colors relative"
      ...
      <main className="flex-grow pt-16">
        ...
        <section id="products-section" className="py-16 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto">
          ...
          {loading ? (
            <div className="text-center py-20 text-xl font-bold text-deepgreen">Loading products...</div>
          ) : error ? (
            <div className="text-center py-20 text-xl font-bold text-red-500">{error}</div>
          ) : (
              ...
              {filteredProducts.map((product) => (
                ...
                >
                  ...
                  <div className="p-5 flex flex-col flex-grow">
                    ...
                    <div className="text-sm text-gray-500 mb-4">{product.unitSize || product.unit}</div>
                    
                    <div className="mt-auto flex items-center justify-between">
                      <span className="text-xl font-extrabold text-deepgreen">₹{product.basePricePKR || product.price}</span>
                      <button 
                        onClick={() => addToCart(product)}
                        className="bg-mintwhite text-vibrantgreen p-2 rounded-full hover:bg-vibrantgreen hover:text-white transition-colors"
                        title="Add to Cart"
                      >
